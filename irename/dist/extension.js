...
// src/commands/renameIdentifier.ts
var vscode = __toESM(require("vscode"));
async function renameIdentifier(uri, line, char, num_tokens) {
  ...
  try {
    const response = await axios_default.post("http://127.0.0.1:8000/rename/", { code, line, char, num_tokens });
...
function findOccurrencesInDocument(document2, line, char) {
  ...
  for (let i = 0; i < document2.lineCount; i++) {
    ...
    while (index !== -1) {
      ...
      const isSameWord = document2.getText(range) === targetWord;
      const isSamePosition = i === line2.lineNumber && index === wordRange.start.character;
      if (isSameWord && isSamePosition) {
        occurrences.push(range);
      } else if (isSameWord && !isSamePosition) {
        occurrences.push(range);
      }
      index = line2.text.indexOf(targetWord, index + targetWord.length);
    }
  }
  return occurrences;
}
